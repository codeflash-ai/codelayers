...
async def _run_ingest(repo_path: Path, output_db: Optional[Path], quiet: bool):
    ...
    if not quiet:
        console.print(
            Panel.fit(
                f"[bold cyan]Starting ingestion of:[/] {repo_path}", border_style="cyan"
            )
        )
        ...
        async for event in run_ingestion(repo_path, output_db):
            if isinstance(event, IngestionProgress):
                ...
                if not quiet:
                    ...
                    result = event.result
                    console.print(
                        f"✓ Indexed {result.files_processed} files → {result.db_path}"
                    )
                ...
                console.print()
                console.print(
                    f"[bold red]✗ Error during {event.stage}:[/] {event.error}"
                )
                sys.exit(1)
...
async def _run_query(query_text: str, db_path: Optional[Path], markdown_output: bool):
    ...
    if db_path is None:
        ...
        if len(db_files) > 1:
            console.print(
                f"[yellow]Found {len(db_files)} databases, using: {db_path.name}[/]"
            )
    ...
    with console.status(f"[cyan]Searching {db_path.name}...[/]"):
        try:
            conversation = await create_conversation(str(db_path), ConversationMessage)
